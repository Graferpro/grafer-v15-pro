// --- VERSİYON (FIX: TICKER CSS + CONVERTER LANG) ---
...
const I18N = {
    ...
    tr: { dark_mode: "Gece Modu", dashboard: "Piyasa", portfolio: "Portföy", crypto: "Kripto", converter: "Çevirici", settings: "Ayarlar", market: "Piyasa", edit: "Düzenle", total_asset: "TOPLAM VARLIK", add: "Ekle", reset: "Sıfırla", crypto_assets: "Kripto Varlıklar", theme_color: "Tema Rengi", default_currency: "Varsayılan Para Birimi", ai_analysis: "AI Analiz", ai_title: "Grafer Pro Ai Asistan", ai_subtitle: "Piyasa Analizi", close: "Kapat", analyzing: "Analiz ediliyor...", enter_amount: "Miktarı Girin...", result: "SONUÇ", quantity_title: "Miktar" },
    pl: { dark_mode: "Tryb ciemny", dashboard: "Rynek", portfolio: "Portfel", crypto: "Krypto", converter: "Przelicznik", settings: "Ustawienia", market: "Rynek", edit: "Edytuj", total_asset: "AKTYWA OGÓŁEM", add: "Dodaj", reset: "Reset", crypto_assets: "Aktywa Krypto", theme_color: "Kolor motywu", default_currency: "Domyślna Waluta", ai_analysis: "Analiza AI", ai_title: "Grafer Pro Ai Asystent", ai_subtitle: "Analiza Rynkowa", close: "Zamknij", analyzing: "Analizowanie...", enter_amount: "Wpisz kwotę...", result: "WYNIK", quantity_title: "Ilość" },
    ru: { dark_mode: "Темная тема", dashboard: "Рынок", portfolio: "Портфель", crypto: "Крипто", converter: "Конвертер", settings: "Настройки", market: "Рынок", edit: "Изменить", total_asset: "ВСЕГО АКТИВОВ", add: "Добавить", reset: "Сброс", crypto_assets: "Криптоактивы", theme_color: "Цвет темы", default_currency: "Валюта по умолчанию", ai_analysis: "AI Анализ", ai_title: "Grafer Pro Ai Помощник", ai_subtitle: "Технический анализ", close: "Закрыть", analyzing: "Анализ...", enter_amount: "Введите сумму...", result: "РЕЗУЛЬТАТ", quantity_title: "Количество" },
    ka: { dark_mode: "ღამის რეჟიმი", dashboard: "ბაზარი", portfolio: "პორტფოლიო", crypto: "კრიპტო", converter: "კონვერტერი", settings: "პარამეტრები", market: "ბაზარი", edit: "რედაქტირება", total_asset: "სულ აქტივები", add: "დამატება", reset: "განულება", crypto_assets: "კრიპტო აქტივები", theme_color: "თემის ფერი", default_currency: "ნაგულისხმევი ვალუტა", ai_analysis: "AI ანალიზი", ai_title: "Grafer Pro AI ასისტენტი", ai_subtitle: "ბაზრის ანალიზი", close: "დახურვა", analyzing: "ანალიზი...", enter_amount: "შეიყვანეთ თანხა...", result: "შედეგი", quantity_title: "რაოდენობა" }
};

const FALLBACK_NEWS = {
    en: [{text: "Global markets update."}, {text: "Bitcoin holds strong."}, {text: "Gold prices fluctuating."}, {text: "Grafer Pro v2.6 Live."}],
    tr: [{text: "Küresel piyasalarda son durum."}, {text: "Bitcoin güçlü duruyor."}, {text: "Altın fiyatlarında dalgalanma."}, {text: "Grafer Pro v2.6 Yayında."}],
    pl: [{text: "Aktualizacja rynków globalnych."}, {text: "Bitcoin trzyma się mocno."}, {text: "Ceny złota się wahają."}, {text: "Grafer Pro v2.6 Działa."}],
    ru: [{text: "Обновление мировых рынков."}, {text: "Биткойн держится уверенно."}, {text: "Цены на золото колеблются."}, {text: "Grafer Pro v2.6 доступен."}],
    ka: [{text: "გლობალური ბაზრების განახლება."}, {text: "ბიტკოინი მყარად დგას."}, {text: "ოქროს ფასების ცვალებადობა."}, {text: "Grafer Pro v2.6 მზადაა."}]
};
...
window.onload = async () => {
    ...
    if(langBtn) {
        ...
        langBtn.style.border = "2px solid rgba(99, 102, 241, 0.3)";
        langBtn.style.borderRadius = "12px";
...
function startNewsTicker() {
    ...
    // CSS'te 'ticker' class'ı flex ve nowrap olduğu için burası yan yana dizecek
...
function setLanguage(lang) {
    ...
    document.getElementById('lang-dropdown').classList.remove('flex');
}
...
function openChartModal(symbol) {
    ...
    modal.innerHTML = `<div class="flex justify-between items-center p-4 border-b border-gray-800 bg-[#131722] relative z-[60]"><h3 id="tv-title" class="text-white font-bold text-lg">${symbol} / USD</h3><button onclick="document.getElementById('tv-modal').remove()" class="text-gray-400 hover:text-white p-2 cursor-pointer"><i data-lucide="x" size="24"></i></button></div><div id="tv-chart-container" class="flex-1 w-full h-full bg-black relative z-0 pb-20"></div><div class="fixed bottom-0 left-0 w-full p-4 bg-[#131722]/95 border-t border-gray-800 z-[100] flex justify-center backdrop-blur-md"><button onclick="openProAIChat('${symbol}')" class="w-full max-w-sm bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-lg transition transform active:scale-95 cursor-pointer"><i data-lucide="sparkles"></i> Grafer Pro Ai</button></div>`;
    document.body.appendChild(modal); lucide.createIcons();
    let tvSymbol = symbol==='USD'?"FX:EURUSD":symbol==='EUR'?"FX:EURUSD":symbol==='TRY'?"FX:USDTRY":symbol==='GBP'?"FX:GBPUSD":symbol==='XAU'?"OANDA:XAUUSD":symbol==='BTC'?"BINANCE:BTCUSDT":`FX:USD${symbol}`;
    if(window.TradingView) new TradingView.widget({ "autosize": true, "symbol": tvSymbol, "interval": "D", "timezone": "Etc/UTC", "theme": "dark", "style": "1", "locale": "en", "toolbar_bg": "#f1f3f6", "enable_publishing": false, "hide_side_toolbar": false, "allow_symbol_change": true, "container_id": "tv-chart-container" });
}
function openProAIChat(symbol) {
    ...
    chatModal.innerHTML = `<div class="bg-white dark:bg-[#1e222d] w-full max-w-md h-[550px] rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden relative animate-fade-in-up"><div class="bg-indigo-600 p-4 flex justify-between items-center text-white shrink-0"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center"><i data-lucide="bot" size="24"></i></div><div><h3 class="font-bold text-base">Grafer Pro Ai</h3><p class="text-[10px] opacity-80 uppercase tracking-wide">Online • Global</p></div></div><button onclick="document.getElementById('pro-chat-modal').remove()" class="hover:text-gray-200 cursor-pointer p-2"><i data-lucide="x" size="24"></i></button></div><div id="pro-chat-messages" class="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50 dark:bg-black/20 text-sm"></div><div class="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1e222d] flex gap-2 shrink-0"><input type="text" id="pro-chat-input" placeholder="..." class="flex-1 bg-slate-100 dark:bg-black/10 border-none rounded-xl px-4 py-3 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" onkeypress="handleProEnter(event)"><button onclick="sendProMessage()" class="bg-indigo-600 hover:bg-indigo-700 text-white p-3 rounded-xl transition cursor-pointer"><i data-lucide="send" size="20"></i></button></div></div>`;
    document.body.appendChild(chatModal); lucide.createIcons(); addProMessage(`Grafer Pro AI: Ready! Analyzing ${symbol}...`, 'bot', true); askOpenAI(`${symbol} (Price: ${price}) technical analysis.`, true);
}
...
function setTheme(color) { state.theme = color; localStorage.setItem('theme', color); document.documentElement.style.setProperty('--theme-color', color); document.querySelectorAll('.theme-dot').forEach(d => d.classList.remove('active')); if(color==='#4f46e5') document.querySelectorAll('.theme-dot')[0].classList.add('active'); else if(color==='#8b5cf6') document.querySelectorAll('.theme-dot')[1].classList.add('active'); else if(color==='#10b981') document.querySelectorAll('.theme-dot')[2].classList.add('active'); else if(color==='#f97316') document.querySelectorAll('.theme-dot')[3].classList.add('active'); else if(color==='#ef4444') document.querySelectorAll('.theme-dot')[4].classList.add('active'); initChart('mainChart', state.theme); }
...
function renderGrid() { const container = document.getElementById('dashboard-grid'); const sym = getSymbol(state.baseCurrency); if(state.rates['XAU'] && !state.favs.includes('XAU')) { state.favs.push('XAU'); } container.innerHTML = state.favs.map(curr => { const val = getPrice(curr); const flagUrl = getFlagUrl(curr); let imgTag = ''; if (curr === 'XAU') imgTag = `<div class="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center text-yellow-600 border border-yellow-200"><i data-lucide="coins" size="16"></i></div>`; else if (curr === 'XAG') imgTag = `<div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 border border-slate-200"><i data-lucide="disc" size="16"></i></div>`; else if (flagUrl) imgTag = `<img src="${flagUrl}" class="w-8 h-8 rounded-full border border-gray-200 dark:border-gray-600 shadow-md">`; else imgTag = `<div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-[10px] border border-slate-200">${curr.substring(0,2)}</div>`; return `<div onclick="openChartModal('${curr}')" class="relative cursor-pointer bg-white dark:bg-cardDark p-4 rounded-2xl neon-box card-pop flex flex-col gap-2 shadow-sm active:scale-95 transition group"><div class="absolute top-3 right-3 text-indigo-500 dark:text-indigo-400"><i data-lucide="maximize-2" size="16"></i></div><div class="flex justify-between items-start">${imgTag}<span class="text-[10px] bg-green-100 text-green-600 px-2 py-0.5 rounded-full font-bold">+0.4%</span></div><div><p class="font-bold text-slate-500 text-xs">${curr}/${state.baseCurrency}</p><p class="font-bold text-xl text-slate-800 dark:text-white">${sym} ${val.toLocaleString(undefined, {maximumFractionDigits:3})}</p></div></div>`; }).join(''); lucide.createIcons(); }
