// --- SÜRÜM KONTROLÜ (CACHE TEMİZLEMEK İÇİN) ---
const APP_VERSION = '1.1'; // Bunu değiştirdikçe kullanıcı sıfırlanır
...
// --- BAŞLANGIÇ (HARD RESET & LOGIC) ---
window.onload = async () => {
    // 1. Sürüm Kontrolü ve Cache Temizliği
    const savedVer = localStorage.getItem('app_version');
    if (savedVer !== APP_VERSION) {
        console.log("Yeni sürüm algılandı! Temizlik yapılıyor...");
    ...
    // 2. Kayıtlı verileri geri yükle
    loadStateFromStorage();

    lucide.createIcons();
    
    // 3. DİL AYARI (TELEFONUN DİLİNE GÖRE)
    // Eğer localStorage'da kayıtlı dil yoksa, telefonun dilini al
    if (!localStorage.getItem('lang')) {
        const deviceLang = navigator.language.slice(0, 2); // 'tr', 'pl', 'en'
        state.lang = I18N[deviceLang] ? deviceLang : 'en'; // Destekliyorsak yap, yoksa İngilizce
    ...
    await fetchData(); 
    await detectLocationCurrency(); // Konuma göre para birimi seç
...
// --- KONUM VE PARA BİRİMİ (Konum: Polonya -> Para: PLN) ---
async function detectLocationCurrency() {
    // Eğer kullanıcı daha önce para birimini değiştirdiyse ona dokunma
    if (localStorage.getItem('user_currency_set')) return;

    try {
        const geoRes = await fetch('https://ipapi.co/json/');
        const geoData = await geoRes.json();
        const localCurr = geoData.currency; // Örn: PLN, TRY
...
async function fetchData() { 
    try { 
        ...
        try {
            ...
            console.log("Altın API hatası, Demo Modu aktif");
...
// --- GRAFİK PENCERESİ VE AI BUTONU (GÜÇLENDİRİLMİŞ) ---
function openChartModal(symbol) {
    let modal = document.getElementById('tv-modal');
    // Her seferinde yeniden yarat (Temiz event listeners için)
...
// --- GRAFER PRO AI ASİSTAN (CHAT PENCERESİ) ---
function openProAIChat(symbol) {
    const price = getPrice(symbol).toFixed(4);
    
    let chatModal = document.getElementById('pro-chat-modal');
    if(chatModal) chatModal.remove(); // Temizle ve yeniden yap

    chatModal = document.createElement('div');
    chatModal.id = 'pro-chat-modal';
    // Z-Index 99999 (EN ÜSTÜN DE ÜSTÜ)
    chatModal.className = 'fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4';
    chatModal.innerHTML = `
        <div class="bg-white dark:bg-[#1e222d] w-full max-w-md h-[500px] rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden relative">
            <div class="bg-indigo-600 p-4 flex justify-between items-center text-white shrink-0">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h3 class="font-bold text-sm">Grafer Pro Ai Asistan</h3>
                        <p class="text-[10px] opacity-80">Online • ${state.lang.toUpperCase()}</p>
                    </div>
                </div>
                <button onclick="document.getElementById('pro-chat-modal').remove()" class="hover:text-gray-200 cursor-pointer"><i data-lucide="x" size="20"></i></button>
            </div>
            ...
            <div class="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1e222d] flex gap-2 shrink-0">
                <input type="text" id="pro-chat-input" placeholder="..." class="flex-1 bg-slate-100 dark:bg-black/10 border-none rounded-lg px-3 py-2 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" onkeypress="handleProEnter(event)">
    ...
    const initialPrompt = `${symbol} (Fiyat: ${price}) hakkında kısa analiz yapar mısın?`;
    addProMessage(`Grafer Pro AI: ${symbol} analizi hazırlanıyor...`, 'bot', true);
...
async function askOpenAI(message, isInitial) {
    ...
    try {
        const res = await fetch('/api/ai', {
            ...
            body: JSON.stringify({ 
                message: message,
                lang: state.lang // Frontend'in o anki dilini gönderiyoruz
            })
        });
        const data = await res.json();
        ...
        if(loadingId) document.getElementById(loadingId).remove();
        addProMessage("Hata: " + e.message, 'bot');
    }
}
...
function startNewsTicker() {
    ...
    const msgs = NEWS_DATA[state.lang] || NEWS_DATA['en']; // Eğer dil yoksa İngilizce göster
...
function startLiveSimulations() {
    ...
    intervals['main'] = setInterval(() => {
        ...
        mainChart.update();
        document.getElementById('chart-price').innerText = `${getSymbol(state.baseCurrency)} ${next1.toLocaleString(undefined, {maximumFractionDigits:4})}`;
    ...
    intervals['crypto'] = setInterval(() => {
        ...
        document.getElementById('crypto-chart-price').innerText = `${getSymbol(state.baseCurrency)} ${next.toLocaleString(undefined, {maximumFractionDigits:2})}`;
    }, 1000);
