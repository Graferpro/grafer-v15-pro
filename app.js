// --- VERSİYON (FIX: GEORGIAN + CONVERTER + NEWS) ---
...
// DİL DESTEĞİ (GÜRCÜCE EKLENDİ + ÇEVİRİCİ DÜZELTİLDİ)
const I18N = {
...
// YEDEK HABERLER (DİLE GÖRE DEĞİŞİR)
const FALLBACK_NEWS = {
    en: [{text: "Global markets update."}, {text: "Bitcoin holds strong."}, {text: "Gold prices fluctuating."}, {text: "Grafer Pro v2.5 Live."}],
    tr: [{text: "Küresel piyasalarda son durum."}, {text: "Bitcoin güçlü duruyor."}, {text: "Altın fiyatlarında dalgalanma."}, {text: "Grafer Pro v2.5 Yayında."}],
    pl: [{text: "Aktualizacja rynków globalnych."}, {text: "Bitcoin trzyma się mocno."}, {text: "Ceny złota się wahają."}, {text: "Grafer Pro v2.5 Działa."}],
    ru: [{text: "Обновление мировых рынков."}, {text: "Биткойн держится уверенно."}, {text: "Цены на золото колеблются."}, {text: "Grafer Pro v2.5 доступен."}],
    ka: [{text: "გლობალური ბაზრების განახლება."}, {text: "ბიტკოინი მყარად დგას."}, {text: "ოქროს ფასების ცვალებადობა."}, {text: "Grafer Pro v2.5 მზადაა."}]
};
...
window.onload = async () => {
    ...
    setLanguage(state.lang); // Haberler ve Placeholder burada ayarlanır
    
    // Dil İkonu Vurgusu (Kullanıcı İsteği)
    ...
    if(langBtn) {
        ...
        langBtn.style.boxShadow = "0 0 10px rgba(99, 102, 241, 0.2)";
    }
    ...
    // Çevirici Reset (Sayfa Açılışında Boş Olsun)
    document.getElementById('conv-amount').value = ''; 
    convert(); // UI'ı sıfırla
...
async function fetchRealNews() {
    try {
        ...
        if (data.news && data.news.length > 0) {
            ...
            state.liveNews = []; // API boşsa fallback kullanacağız
        }
    } catch (e) {
        console.log("Haber çekilemedi, yedekler devrede.");
        state.liveNews = [];
    }
    startNewsTicker(); // Haberleri başlat veya güncelle
}

// --- HABER BANDI (DÜZELTİLDİ & YERELLEŞTİRİLDİ) ---
function startNewsTicker() {
    const container = document.getElementById('news-ticker');
    
    // Eğer canlı haber varsa onu kullan, yoksa SEÇİLİ DİLİN yedek haberlerini kullan
...
function getPrice(code) { 
    ...
    if(!rateCode) { 
        ...
        else if(code==='XAU') rateCode = 1/2740; 
        else if(code==='XAG') rateCode = 1/32.50; 
        else rateCode = 1; 
    } 
...
async function askOpenAI(message, isInitial) {
    ...
    try {
        ...
        const data = await res.json();
        if(loadingId) document.getElementById(loadingId).remove(); if(isInitial) container.innerHTML = ''; 
...
function setTheme(color) {
    ...
    if(color==='#4f46e5') document.querySelectorAll('.theme-dot')[0].classList.add('active'); else if(color==='#8b5cf6') document.querySelectorAll('.theme-dot')[1].classList.add('active'); else if(color==='#10b981') document.querySelectorAll('.theme-dot')[2].classList.add('active'); else if(color==='#f97316') document.querySelectorAll('.theme-dot')[3].classList.add('active'); else if(color==='#ef4444') document.querySelectorAll('.theme-dot')[4].classList.add('active');
    initChart('mainChart', state.theme);
}
...
function initChart(id, color) {
    const canvas = document.getElementById(id); if(!canvas) return; if(charts[id]) charts[id].destroy(); 
...
function startLiveSimulations() {
    ...
    if(mainChart) {
        ...
        intervals['main'] = setInterval(() => {
            ...
            if(state.isChartSwapped) { document.getElementById('chart-symbol').innerHTML = `<span class="base-curr-text">${state.baseCurrency}</span>/${state.chartPair}`; } else { document.getElementById('chart-symbol').innerHTML = `${state.chartPair}/<span class="base-curr-text">${state.baseCurrency}</span>`; }
        }, 1000);
    }
    ...
    if(cChart) {
        ...
        intervals['crypto'] = setInterval(() => {
            const arr = cChart.data.datasets[0].data; const next = arr[arr.length-1] * (1 + (Math.random()-0.5)*0.01); arr.shift(); arr.push(next); cChart.update(); const sym = getSymbol(state.baseCurrency); document.getElementById('crypto-chart-price').innerText = `${sym} ${next.toLocaleString(undefined, {maximumFractionDigits:2})}`;
        }, 1000);
    }
    const flag = getFlagUrl(state.chartPair); const chartFlag = document.getElementById('chart-flag'); if(flag) { chartFlag.src = flag; chartFlag.style.display='block'; } else { chartFlag.style.display='none'; }
    document.getElementById('crypto-chart-symbol').innerHTML = `${state.cryptoChartPair}/<span class="base-curr-text">${state.baseCurrency}</span>`; document.getElementById('crypto-chart-icon').src = `https://assets.coincap.io/assets/icons/${CRYPTO_ICONS[state.cryptoChartPair]||'btc'}@2x.png`;
}
...
function renderDrawerList(items, activeList, isSingleSelect) {
    document.getElementById('drawer-list').innerHTML = items.map(c => {
        const isSel = activeList.includes(c); let img = ''; let name = CURRENCY_NAMES[c] || c;
        if(CRYPTO_ICONS[c]) img = `<img src="https://assets.coincap.io/assets/icons/${CRYPTO_ICONS[c]}@2x.png" class="w-8 h-8 rounded-full object-cover bg-white">`; else if (c === 'XAU') img = `<div class="w-8 h-8 rounded-full bg-yellow-400 flex items-center justify-center text-white"><i data-lucide="coins" size="14"></i></div>`; else if (c === 'XAG') img = `<div class="w-8 h-8 rounded-full bg-slate-400 flex items-center justify-center text-white"><i data-lucide="disc" size="14"></i></div>`; else { const flagUrl = getFlagUrl(c); img = flagUrl ? `<img src="${flagUrl}" class="w-8 h-8 rounded-full shadow-sm border border-slate-100 object-cover">` : `<div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center"><i data-lucide="globe" size="16" class="text-slate-400"></i></div>`; }
        let icon = ''; if (isSingleSelect && state.drawerMode !== 'add-asset') { icon = isSel ? `<i data-lucide="circle-dot" class="text-[${state.theme}]"></i>` : '<i data-lucide="circle" class="text-slate-300"></i>'; } else if (isSel) { icon = `<i data-lucide="check-circle" class="text-[${state.theme}] fill-indigo-100"></i>`; }
...
// --- DÜZELTİLEN CONVERT FONKSİYONU ---
function convert() { 
    const inputVal = document.getElementById('conv-amount').value;
    // Eğer girdi boşsa --- göster
    if(inputVal === '') {
